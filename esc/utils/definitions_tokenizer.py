...
def get_tokenizer(
    transformer_model: str, use_special_tokens: bool
...
class DefinitionsTokenizer:
    ...
    def encode_pair(
        ...
        torch.LongTensor, Optional[List[Tuple[int, int]]], Optional[torch.LongTensor]
    ]:
        encoding_out = self.tokenizer.encode_plus(
            sequence1, sequence2, return_tensors="pt", return_token_type_ids=True
        )
        ...
        return (
            encoding_out.input_ids.squeeze(),
            encoding_out.encodings[0].offsets,
            token_type_ids,
        )
    ...
    def prepare_sample_without_st(
        ...
        encoded_final_sequence, offsets, token_type_ids = self.encode_pair(
            context_sentence, definitions_seq
        )

        context_start_position = [
            i
            for i, elem in enumerate(offsets[:-1])
            if sum(elem) + sum(offsets[i + 1]) == 0
        ][0]
        ...
        definitions_positions = [
            (context_offset + si2ti[gsi], context_offset + ei2ti[gei])
    ...
    def prepare_sample_with_st(
        ...
        definitions_seq = "".join(
            [f"{GLOSS_START_TOKEN} {_def}{GLOSS_END_TOKEN}" for _def in definitions]
        )
        encoded_final_sequence, _, token_typed_ids = self.encode_pair(
            context_sentence, definitions_seq
        )

        # removing spaces created with the addition of <classify> and </classify>
        encoded_final_sequence = encoded_final_sequence[
            encoded_final_sequence != self.space_token_id
        ]

        definitions_positions = zip(
            (
                i
                for i, wid in enumerate(encoded_final_sequence)
                if wid == self.gloss_start_token_id
            ),
            (
                i
                for i, wid in enumerate(encoded_final_sequence)
                if wid == self.gloss_end_token_id
            ),
        )
...
class AutoDefinitionsTokenizer(DefinitionsTokenizer):
    def __init__(self, transformer_model: str, use_special_tokens: bool):
        ...
        if self.use_special_tokens:
            ...
            self.gloss_start_token_id = self.tokenizer.encode(
                "<g>", add_special_tokens=False
            )[0]
            self.gloss_end_token_id = self.tokenizer.encode(
                "</g>", add_special_tokens=False
            )[0]
